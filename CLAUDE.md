...
- **Reference implementation**: [Duckling](https://github.com/l1xnan/duckling) is the primary reference for Tauri + DuckDB IPC patterns. Strip out Parquet, DB connections, and the schema browser (the search bar's `sql` mode covers the SQL-console need in a much narrower form) — keep DuckDB connection management, `read_csv_auto` usage, encoding detection, and the AG-Grid datasource pattern.
...
- **`get_csv_data_range` is sync**: it's a fast, bounded paging query (≤500 rows) that isn't worth a thread hop — not a `Send` restriction. DuckDB's `Connection` *is* `Send` (it is `!Sync`, which the `Arc<Mutex<_>>` already handles); this is what lets `apply_query`/`preview_query` move a `TabSession` onto `spawn_blocking`. Verified against the bundled `duckdb` crate source.
