...
| シナリオ | 表示内容 | 実装状況 |
|---------|---------|---------|
| ファイル読み込み失敗 | ステータスバー + トースト通知 | 未実装（[#36](https://github.com/proxit-inc/csv-viewer/issues/36)スコープ外） |
| エンコーディング検出が低信頼度 | 手動選択の通知バーを提示 | 実装済み（`EncodingNotice`） |
| 不正なCSV形式 | 部分的に読み込んで警告表示 | 未実装（[#36](https://github.com/proxit-inc/csv-viewer/issues/36)スコープ外） |
| 1,000,000 行超 | アプリ内通知バー（続行可） | 実装済み（`LargeFileNotice`） |
| DuckDB接続エラー | タブに ⚠ アイコン表示 | 実装済み（`get_csv_data_range`の接続エラーのみ対象） |
